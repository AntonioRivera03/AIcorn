...
Templates and Jobs share one searchable card grid. Every card has **Edit** and **Convert**; Jobs also have **Run**. Edit opens the autosaving editor on the same page, with **All templates & jobs** returning to the grid. A Job represents its source template in the grid, so conversion does not add a duplicate template card. The source template remains available from the Job editor and the task template picker. Converting a Job back requires confirmation: its schedule and run history are removed, while its template and previously created tasks remain. Active Jobs cannot be converted.
