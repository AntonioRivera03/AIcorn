...
- Migration `00009_personas.sql` — `persona` and `stage_persona` tables.
- Backend CRUD: `personaHandler.go` / `personaService.go` / `personaRepo.go`.
...
CREATE TABLE persona (
  ...
  model          TEXT NOT NULL,
  allowed_tools  TEXT NOT NULL, -- JSON array of MCP tool names, restrictive default
...
**Backend** — wire `personaService`/`personaRepo` into the `app` struct in `server/cmd/web/main.go` and `routes.go`, matching the existing manual-DI pattern. Bulk endpoints return `models.BulkResult` per the root `CLAUDE.md` contract, using `dedupeInts` from `services/util.go`.
...
**The registry principle** — `allowed_tools` makes personas configuration, not hardcoded Go types. Adding a new persona later is a row insert. Make the column `NOT NULL` with a restrictive default: omitting a tools list is a documented anti-pattern that silently grants a persona every tool.
...
- [ ] At least one persona defined with a deliberately narrow `allowed_tools` list (this becomes the Phase 3 read-only research persona).
