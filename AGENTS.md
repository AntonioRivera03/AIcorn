...
- Working in `server/` → also read [`server/AGENTS.md`](server/AGENTS.md) (Go architecture, full DB schema, running/reseeding, backend gotchas).
- Working in `app/` → also read [`app/AGENTS.md`](app/AGENTS.md) (React/TS rules, state, styling, frontend gotchas).
...
```
app/      # React/TypeScript frontend       → app/AGENTS.md
server/   # Go backend + SQLite             → server/AGENTS.md
AGENTS.md # this file (cross-cutting rules, always loaded)
```

The backend and frontend each carry their own `AGENTS.md` that Codex auto-loads when the agent works in that tree. Keep cross-cutting rules here; push layer-specific detail down.
...
- **Flag migration schema implications** whenever a feature changes the allowed values of a hardcoded `CHECK` constraint — `task.priority`, `task.type`, or `stage.type` (see `server/AGENTS.md`).
