...
Templates and Jobs share one searchable vertical list of wide cards. Every card has **Edit** and **Convert**; Jobs also have **Run**. Edit opens the autosaving editor on the same page, with **All templates & jobs** returning to the list. A Job represents its source template in the list, so conversion does not add a duplicate template card. The source template remains available from the Job editor and the task template picker. Converting a Job back requires confirmation: its schedule and run history are removed, while its template and previously created tasks remain. Active Jobs cannot be converted.

In the project's list or kanban, open **New Task → ⋯ → Template…** to search and load a template. This copies its task fields and formatted Markdown into the open editor locally. Choosing a template, or closing it without edits, does not save the copied values, modify the template, create another task, or run a Job. The existing New Task flow still creates the initial empty task immediately. The next ordinary title, property, or body edit autosaves the loaded defaults and body together through the usual task endpoints. Job-only agent instructions are not copied into the task body.

**Create task** stamps a normal task using the template's current content and properties, without starting AI. Subsequent template edits do not change existing tasks. **Convert to job** creates a separate Job that references the template. Choose the task agent that should execute the Job. Built-in roles supply their fixed instructions and selected model. Configure distinct In progress and Human review stages in the project's Conductor settings. Its repository option, working instructions and handoff requirements also apply to Jobs.

**Run now** stamps and queues one task immediately. It does not require a schedule or an enabled board Conductor. Jobs create an independent task session directly in In progress, using the selected task agent. Server code handles ownership and the Human review transition; no dispatcher or subagents are needed to execute a Job. If the result needs more context, add it to the task and use **Recheck**. That preserves the Job's captured agent, model, settings and independent run mode even while the board Conductor remains paused. A completed task retains normal AI history, native Codex session links, file changes, its isolated branch, and preview/merge actions. Completion hands off for human review rather than marking the task Done.
...
The Jobs service tests use a migrated SQLite database and the real queue/worker. They cover template snapshots and scope, concurrent manual idempotency, overlap handling, missed cadence, failed setup, timezone/DST calculations, automatic loop startup/shutdown, and both manual and scheduled task-session-to-review runs while the board toggle is off. Repository runs use temporary Git repositories and verify retained branches and diffs. Recheck tests verify that missing-context recovery preserves the Job agent and prompts. HTTP tests cover CRUD, malformed requests, revision conflicts, queue creation, and preview execution restrictions.

Browser verification uses a disposable database: create and rapidly edit a template, instantiate its body/settings, convert it to a Job, select an agent, edit cron/timezone, display validation errors, and verify the delete confirmation. Actual dispatch, independent task sessions, and same-session follow-ups are covered by the installed-Codex opt-in test documented in [built-in agents](built-in-agents.md).
