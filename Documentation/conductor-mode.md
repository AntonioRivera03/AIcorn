...
One agent runs at a time. There is no automatic retry, merge, container provisioning, spending cap, or parallel worker pool. Command networking is disabled; the proposed [branch environments and task containers](branch-environments-spec.md) describe dependency preparation, isolated preview databases, and future execution profiles. Manual runs still do not control workflow stages. Conductor never moves work directly to Done.
